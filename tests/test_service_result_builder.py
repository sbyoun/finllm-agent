...
class ServiceResultBuilderTest(unittest.TestCase):
    def test_build_result_uses_previous_successful_sql_when_last_sql_is_empty(self) -> None:
        ...
        run_events: list[object] = [
            *_sql_events(
                call_id="positive",
                sql="select name, ticker from stocks where price <= 10000",
        ...
        self.assertEqual(result.sql, "select name, ticker from stocks where price <= 10000")
        self.assertEqual(result.sqlScripts, ["select name, ticker from stocks where price <= 10000"])
