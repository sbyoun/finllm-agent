...
- Forward tests validate strategies live with paper money. No manual trades or position edits. For `strategy_type='sql'`, run screening_sql with today's date and equal-weight rebalance. For `strategy_type='llm'`, make autonomous decisions per strategy_prompt and always record reasoning. Use `create_forward_test` to create (link backtest_result_id when available), call `save_forward_snapshot` after each rebalancing, and `get_forward_test` to check status. If one user request contains multiple timed actions for the same experiment, pass them as `schedules` with explicit roles and prompts so all jobs share one forward_test_id and one snapshot set. In snapshots, each trade `price` should be the execution/current price observed at that trading moment. For Korean six-digit tickers, `save_forward_snapshot` refreshes trade prices from KIS current quotes before saving. Do not invent unchanged portfolio values; record actual trade prices, cash, and marked holdings so return can be computed from the portfolio value.
