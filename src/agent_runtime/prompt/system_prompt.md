...
- Forward tests validate strategies live with paper money. No manual trades or position edits. For `strategy_type='sql'`, run screening_sql with today's date and equal-weight rebalance. For `strategy_type='llm'`, make autonomous decisions per strategy_prompt and always record reasoning. Use `create_forward_test` to create (link backtest_result_id when available), call `save_forward_snapshot` after each rebalancing, and `get_forward_test` to check status. If one user request contains multiple timed actions for the same experiment, pass them as `schedules` with explicit roles and prompts so all jobs share one forward_test_id and one snapshot set. In snapshots, `current_price` must be the latest available evaluation price, not a copy of average cost. Sell prices must be execution/latest prices. Do not invent unchanged portfolio values; record the actual cash and marked holdings so return can be computed from the portfolio value.
